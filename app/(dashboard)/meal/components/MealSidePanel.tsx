...
import { fetchSingleRecord, saveRecord } from '../actions';

interface MealSidePanelProps {
  action: 'new' | 'edit' | null;
  recordId: string | null;
  table: TableConfig;
  packages: { label: string; value: any }[];
  onSuccess: () => void;
}

export default function MealSidePanel({ action, recordId, table, packages, onSuccess }: MealSidePanelProps) {
  ...
  const getDynamicOptions = (optionsKey: string | { label: string; value: any }[]) => {
    if (typeof optionsKey === 'string') {
      if (optionsKey === 'packages') return packages;
      return [];
    }
    return optionsKey;
  };
    ...
    if (isOpen) {
      if (action === 'new') {
        ...
        const fetchRecord = async () => {
          ...
          const data = await fetchSingleRecord(table.dbName, primaryKeyField, recordId);
          if (data) {
            const parsedData: Record<string, any> = {};
            table.fields.forEach(f => {
              const val = data[f.name];
              if (f.type === 'boolean') {
                parsedData[f.name] = val ? 'true' : 'false';
              } else {
  ...
  const handleSave = async (e?: React.FormEvent) => {
    ...
    for (const field of table.fields) {
      if (field.isPrimaryKey && field.name === 'id' && action === 'new') {
        if (field.type === 'text') {
          submissionData[field.name] = crypto.randomUUID();
        }
        continue;
      }
      ...
      if (field.type === 'number') {
        submissionData[field.name] = (value !== '' && value !== undefined && value !== null) ? Number(value) : null;
      } else if (field.type === 'boolean') {
        submissionData[field.name] = String(value) === 'true';
      } else {
    ...
    try {
      const saveRes = await saveRecord(table.dbName, primaryKeyField, submissionData, action === 'edit' ? recordId : null);
      if (!saveRes.success) throw new Error(saveRes.error || 'Failed to save record');
      
      onSuccess();
      handleClose();
    } catch (err) {
      console.error('Error saving record:', err);
      alert(`Failed to save record: ${(err as any).message || JSON.stringify(err)}`);
    } finally {
      setLoading(false);
    }
  };
                ...
                return (
                  <div key={field.name} className="flex flex-col gap-1.5">
                    ...
                    {field.type === 'select' ? (
                      ...
                      <div 
                        className={`w-11 h-6 rounded-full flex items-center px-0.5 transition-colors cursor-pointer ${String(formData[field.name]) === 'true' ? 'bg-blue-600' : 'bg-slate-200'} ${isReadOnly ? 'opacity-50 pointer-events-none' : ''}`}
                        onClick={() => {
                          if (isReadOnly) return;
                          setFormData({ ...formData, [field.name]: String(formData[field.name]) === 'true' ? 'false' : 'true' });
                        }}
                      >
                        <div className={`w-5 h-5 rounded-full bg-white shadow-sm transform transition-transform ${String(formData[field.name]) === 'true' ? 'translate-x-5' : 'translate-x-0'}`} />
                      </div>
                    ) : field.type === 'textarea' ? (
                      <textarea
                        disabled={isReadOnly}
                      ...
                      <input
                        ...
                        disabled={isReadOnly || (field.name === 'price' && String(formData['is_paid']) !== 'true')}
                        value={formData[field.name] || ''}
                        onChange={e => setFormData({ ...formData, [field.name]: e.target.value })}
